class RegExpDiagram {
    ...
    _readStation(pattern, firstIndex = 0) {
        ...
        if (pattern[0] == '(') {
            ...
            if (RegExpDiagram._supportsNamedGroup) {
                ...
                if (result) {
                    ...
                    return { station: {
                        type: 'group',
                        value: this._readStations(pattern.substr(re.lastIndex, i - re.lastIndex), firstIndex + re.lastIndex),
                        groupName
                    }, lastIndex: i + 1 }
                }
            }
...
`
    ...
    _appendStationNode(station, parentNode) {
        if (station.type === 'character' || station.type === 'classified') {
            ...
            if (this.style.drawsNoRememberGroup || (typeof station.groupNumber !== 'undefined') || (typeof station.groupName !== 'undefined') || (typeof station.lookahead !== 'undefined') || (typeof station.lookbehind !== 'undefined')) {
                this._decorateGroup(eznode, station.groupNumber, station.groupName, station.lookahead, station.lookbehind)
            }
            if (station._textRange) {
                eznode.node.setAttribute('_textRange', `${station._textRange.firstIndex},${station._textRange.lastIndex}`)
            }
            ...
            if ((typeof lookahead !== 'undefined') || (typeof lookbehind !== 'undefined')) {
                let annotation = group.append('text', ':first-child').let(it => {
                    ...
                    if (typeof lookahead !== 'undefined') {
                        ...
                        it.textContent = `${lookbehind} lookbehind`
                    }
                    it.setAttribute('x', x0)
                    it.setAttribute('y', y0 - this.style.textPadding.y)
